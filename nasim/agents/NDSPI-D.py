...
attack_path = namedtuple("attack_path", ("path_rewards", "actions"))
...
class TopAttackPath:
    def __init__(self, size=3):
        self.size = size
        self.count = 0
        self.attack_paths: list[attack_path] = []

    def add(self, path: attack_path):
        if len(self.attack_paths) < self.size:
            self.attack_paths.append(path)
            self.attack_paths.sort(key=lambda x: x[0])
        else:
            new_reward = path.path_rewards
            if new_reward > self.attack_paths[-1].path_rewards:
                self.attack_paths.append(path)
            elif new_reward == self.attack_paths[-1].path_rewards and len(
                path.actions
            ) < len(self.attack_paths[-1].actions):
                self.attack_paths.append(path)

            self.attack_paths.sort(key=lambda x: x[0])
            self.attack_paths = self.attack_paths[-self.size :]
        self.count = len(self.attack_paths)

    # def show(self):
    #     pprint(self.attack_paths)


# test = TopAttackPath(size=3)
# p1 = attack_path(path_rewards=2, actions=[1, 2])
# test.add(p1)
# p2 = attack_path(path_rewards=3, actions=[1, 2])
# test.add(p2)
# p3 = attack_path(path_rewards=3, actions=[1, 2])
# test.add(p3)
# p4 = attack_path(path_rewards=9, actions=[1, 2])
# test.add(p4)
# p5 = attack_path(path_rewards=1, actions=[1, 2])
# test.add(p5)
# test.show()
...
class ReplayMemory:
    ...
    def __init__(self, capacity, s_dims, device="cuda"):
        ...
        self.device = device
        self.s_buf = np.zeros((self.capacity, *s_dims), dtype=np.float32)
        self.a_buf = np.zeros((self.capacity, 1), dtype=np.int64)
        self.next_s_buf = np.zeros((self.capacity, *s_dims), dtype=np.float32)
...
class ReplayMemoryPER:
    ...
    def sample(self, n):
        ...
        h = torch.from_numpy(np.vstack(list(batchs[1])).astype(np.int64)).to(
            self.device
        )
        a = torch.from_numpy(np.vstack(list(batchs[2])).astype(np.int64)).to(
            self.device
        )
...
class DQN_PERAgent:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.rewards_step = []
        self.path_record = TopAttackPath(size=3)
        # Neural Network related attributes
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.dqn = DQN_dueling_noisy(
            self.obs_dim, hidden_sizes, self.action_num, self.host_num
        ).to(self.device)
        if self.verbose:
            ...
            print(self.dqn)
        self.num_episodes = 0
        self.target_dqn = DQN_dueling_noisy(
            self.obs_dim, hidden_sizes, self.action_num, self.host_num
        ).to(self.device)
    ...
    def append_sample(self, state, host, action, reward, next_state, done):
        host_q_vals_raw, action_q_vals_raw = self.dqn(
        ...
        host_target_q_val_raw, action_target_q_val_raw = self.target_dqn(
        ...
        if done:
            ...
            target = reward + self.discount * 0.5 * (
                host_target_q_val + action_target_q_val
            )
    ...
    def get_egreedy_action(self, o, epsilon):
        if random.random() > epsilon:
            ...
            a = a.cpu().item()
            return h, a
        return random.randint(0, self.host_num - 1), random.randint(
            0, self.action_num - 1
        )

    def getV(self, q_value):
        v = self.alpha * torch.log(
            (1 / self.alpha * q_value).exp().sum(dim=-1, keepdim=True)
        )
        vv = self.alpha * torch.logsumexp(q_value / self.alpha, dim=-1)  # YOUR CODE
        return vv

    def optimize(self):
        s_batch, h_batch, a_batch, next_s_batch, r_batch, d_batch, idxs, is_weight = (
            self.replayPER.sample(self.batch_size)
        )
        ...
        with torch.no_grad():
            ...
            host_target_q_val_raw2 = self.alpha_host * torch.logsumexp(
                host_target_q_val_raw / self.alpha_host, dim=-1
            )
            action_target_q_val_raw2 = self.alpha_action * torch.logsumexp(
                action_target_q_val_raw / self.alpha_action, dim=-1
            )
        ...
        if torch.cuda.is_available():
            loss = (
                torch.cuda.FloatTensor(is_weight)
                * F.mse_loss(q_vals, target, reduce=False, reduction="none")
            ).mean()
        else:
            loss = (
    ...
    def train(self):
        ...
        while self.num_episodes < self.episode_number:
            ...
            if self.replayPER.tree.n_entries >= self.train_start:
                ...
                self.logger.add_scalar("return-steps", ep_return, self.steps_done)
                ...
                print(
                    f"\treplay memory: = {self.replayPER.tree.n_entries} / "
                    f"{self.train_start}"
                )
        ...
        if self.verbose:
            ...
            print(f"\treturn = {ep_return}")
            print(f"\tgoal = {goal}")
        print("Top 3 Penetration testing paths: ")
        for path in self.path_record.attack_paths:
            pprint("="*10)
            print(f"Scores: {path.path_rewards}")
            pprint("-"*10)
            print(f"Path: ")
    ...
    def run_train_episode(self, step_limit):
        ...
        while not done:
            ...
            if self.replayPER.tree.n_entries >= self.train_start:
                ...
                self.steps_done += 1
                steps += 1
                self.logger.add_scalar("epsilon", eee, self.num_episodes)
        ...
        episode_ap = attack_path(path_rewards=episode_return, actions=action_set)
        self.path_record.add(episode_ap)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--replay_size", type=int, default=1e5, help="(default=100000)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--exploration_steps", type=int, default=200, help="(default=10000)"
    )
    parser.add_argument("--gamma", type=float, default=0.9, help="(default=0.99)")
