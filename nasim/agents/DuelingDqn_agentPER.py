...
class DuelingDQN(nn.Module):
    ...
    def __init__(self, input_dim, layers, num_actions):
        ...
        self.advantage_stream=nn.Linear(layers[-1], num_actions)
...
class DuelingDQN2(nn.Module):
    ...
    def forward(self, x):
        for layer in self.layers:
            x = F.relu(layer(x))        
        #x = self.out(x)
        values = F.relu(self.value_hidden_layer(x))
...
class DuelingDQN_PERAgent:
    ...
    def __init__(self,
        ...
        self.dqn = DuelingDQN2(self.obs_dim,
                       hidden_sizes,
        ...
        if self.verbose:
            ...
            print(self.dqn)
        self.num_episodes = 0
        self.target_dqn = DuelingDQN2(self.obs_dim,
                              hidden_sizes,
                              self.num_actions).to(self.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--training_steps", type=int, default=500000,
    ...
    parser.add_argument("--init_epsilon", type=float, default=1.0,
                        help="(default=1.0)")
    parser.add_argument("--exploration_steps", type=int, default=150000,
                        help="(default=10000)")
    ...
    env = nasim.make_benchmark(args.env_name,
                               ...
                               flat_actions=True,
                               flat_obs=True)
    dqn_agent = DuelingDQN_PERAgent(env, verbose=args.quite, **vars(args))
    dqn_agent.train()
    dqn_agent.save("D:\\Experiments\\NetworkAttackSimulator\\medium-multi-site-honeypot\\Dueling_PER_Mar-8.pkl")
    #save_data(dqn_agent.rewards_episode,'D:\\Experiments\\NetworkAttackSimulator\\medium-multi-site\\Dueling_PER_rewards_episode_Feb26-2.csv')
    #save_data(dqn_agent.rewards_step,'D:\\Experiments\\NetworkAttackSimulator\\medium-multi-site\\Dueling_PER_rewards_step_Feb26-2.csv')
