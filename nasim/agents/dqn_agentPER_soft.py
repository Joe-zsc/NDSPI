...
class DQN_PERAgent:
    ...
    def get_egreedy_action3(self, o, epsilon):
        if random.random() > epsilon:
            o = torch.from_numpy(o).float().to(self.device)
            q_vals_raw = self.dqn(o)
            q_vals=self.getV(q_vals_raw)
            pi_maxent = torch.exp((q_vals_raw - q_vals) / self.alpha)
    ...
    def run_train_episode(self, step_limit):
        ...
        while not done :
            ...
            a = self.get_egreedy_action3(o, eee)
            action_set.append(a)
